...
class ModbusRTUSlave {
  public:
    ...
    void begin(HardwareSerial& serial, uint32_t baud, uint8_t config = SERIAL_8N1, uint8_t dePin = NO_DE_PIN);
    #ifdef __AVR__
    void begin(SoftwareSerial& serial, uint32_t baud, uint8_t dePin = NO_DE_PIN);
    #endif
    #ifdef HAVE_CDCSERIAL
    void begin(Serial_& serial, uint32_t baud);
    #endif
